...
大多数包含flex词法分析器的程序使用词法分析器来获取一个记号流，这样可以方便语法分析器的处理。每当程序需要一个记号时，它调用yylex()来读取一小部分输入然后返回相应的记号，当程序需要下一个记号时，yylex（）会被再次调用。词法分析器以系统程序的方式来运行，也就是说每次它返回的时候，他会记住当前处理的位置，并从这个位置开始去处理下一次调用。

在词法分析器中，当动作代码识别出一个记号时，yyelex()将这个标记作为返回值，当程序再次调用yylex（）时候，词法分析器会以后续的输入字符继续分析，相反，如果一个模式不能够产生一个用于调用程序且不可以返回记号时，词法分析器会在这次yylex()的调用中继续分析。下面这个并不完整的片段包含了两个可以返回记号的模式，一个返回+操作符，另一个返回数字，他还包含了一个不做任何事情的空白字符的模式，这个模式用来忽略它所匹配的内容。
...
这种看起来并不确定的返回方式经常会迷惑flex的新手。但是这种规则事实上非常简单，如果动作有返回，词法分析会在下一次yylex()调用时继续，如果没有返回此法分析器将会立即继续执行。

现在我们修改我们的词法分析器，它所返回的记号可以被语法分析器用来实现一个计算器。

#### 1.2.5 记号编号和记号值

当flex词法分析器返回一个记号流时候，每个记号实际有两个部分，记号的编号和记号值，这里的记号编号是一个较小的整数，这个数字是可以随意指定的，但是零值总是意味着文件的结束，当bison创建一个词法分析器时，bison自动从258起指派每个标记的编号，并且创建一个包含这些编号定义的.h文件，不过这次我们手工定义一些编号:
...
记号值可以区分一组相似的记号，在我们词法分析器中，所有的数字都属于Number这个标号，而记号值则表明了具体的数字。当分析更复杂的输入时，（名称，浮点数，字符串变量等等），记号值将告诉我们具体的名称，数字，字面值和其他内容。我们第一版的计算器吃饭分析器带有一个用于调试的主程序。参见例1-4。

例1-4：计算器词法分析器fb1-4.1
